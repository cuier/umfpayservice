...
function umfService() {
    ...
    this.getToken = function(cb){
        ...
        merMap["client_id"] = "79756098a6d291260523f6199ddfc4bc0875ccf6"
        merMap["client_secret"] = "f642ab9bd929ab0d7f802e12dc689e416d4b8e6a"
        let url = Constants.UMF_RESTPAY_AUTHORIZE
        let headers =  {
            'Content-Type': 'application/json',
        }
        netWorkUtil.childMerQuest(url,JSON.stringify(merMap),headers,(res)=>{
            return typeof cb == "function" && cb(res)
        })
    ...
    this.addChildMerInfo = function (token,RequestMap,cb) {
        ...
        delete params["sign"]
        netWorkUtil.childMerQuest(url,JSON.stringify(params),headers,(res)=>{
            return typeof cb == "function" && cb(res)
        })
    }
    ...
    this.selectChildMerState = function(token,reqMap,cb){
        ...
        netWorkUtil.childMerQuest(url,"",headers,(res)=>{
            return typeof cb == "function" && cb(res)
        })
    }
}
